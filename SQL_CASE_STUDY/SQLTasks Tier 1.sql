...
SELECT name AS facility_name, CONCAT(firstname, ' ' , surname) AS member_name, subquery.booking_cost
FROM 
...
WHERE subquery.booking_cost > 30
ORDER BY booking_cost DESC;
...
SELECT CONCAT(m1.surname, ', ' , m1.firstname) AS member_name, CONCAT(m2.surname, ', ' , m2.firstname) AS referring_member
FROM Members m1
...
SELECT facid, name AS facility_name, COUNT(memid) AS member_usage
FROM Bookings
JOIN Facilities
USING (facid)
WHERE memid != 0
...
SELECT DATE_FORMAT(b.starttime, "%M %Y") as usage_month, f.facid, f.name AS facility_name, COUNT(b.memid) AS member_usage
FROM Bookings b
JOIN Facilities f ON f.facid = b.facid
WHERE b.memid != 0
GROUP BY DATE_FORMAT(b.starttime, "%M"), f.name, f.facid
ORDER BY member_usage DESC;